...
def get_lifecycle_command():
    ...
    total = 0
    #print(lifecycle_set)
    for lifecycle in lifecycle_set:
        print(lifecycle_set[lifecycle])
        now = 1
        for technique in lifecycle_set[lifecycle]:
            if technique in technique_command and technique_command[technique] != []:
                # 隨機選一個 procedure
                now = now * len(technique_command[technique])
        print(lifecycle, now)
        if now > 50:
            now = 50
        total = total + now
