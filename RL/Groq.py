...
class Groq_LLM:
    def __init__(self, model_name="llama-3.1-8b-instant"):
        ...
        self.SYSTEM_PROMPT = "I want you to act as a ubuntu terminal which have join into ad domain 'hslab.com'. I will type commands and you will reply with what the terminal should show. I want you only to reply with the terminal output in plaintest and nothing else. Do not write explanations. Do not type commands unless I instruct you to do so. All the software have already install. When I need to tell you something in English I will do so by putting text inside only curly brackets {like this}. You should print the terminal output first and final line is the prompt for input."
        # 記得金鑰不要洩漏出去
        self.llm = OpenAI(api_key = api_key)

        print("Loaded Model: ", self.BASE_MODEL_NAME)

    def answer(self, action, query, log_history=[]):
    ...
    def detect_honeypot(self, log_history=[]):
    ...
    def next_state(self, action, query, log_history=[]):
