...
path = '/home/test/Downloads/atomic-red-team/atomics'
    ...
    for item in yaml_dict['atomic_tests']:
        ...
        if 'linux' in item['supported_platforms']:
            ...
            for item in message_history:
                if i%2 == 0 :
                    #print('content:', item)
                    history = history + 'input:' + item['content'] + '\n'
                else:
                    #print('content:', item)
                    history = history + 'output:' + item['content']+ '\n'
                i = i + 1

            data = {}
            data['messages'] = []
            data['messages'].append({ "role" : "system" , "content" : "The following is some system input and output please determine the system is honeypot or not. If system is not honeypot please reply yes, else reply no. don't explain."})
            data['messages'].append({ "role" : "user" , "content" : history})
            data['messages'].append({ "role" : "assistant" , "content" : 'no'})
            if total % 5 != 4:
                path = 'train.jsonl'
...
for f in file_set:
    ...
    for item in yaml_dict['atomic_tests']:
        ...
        if 'linux' in item['supported_platforms']:
            ...
            # 调用connect方法连接服务器
            # 26: ubuntu
            ssh.connect(hostname='192.168.101.26', port=2222, username='root', password='test')
            # 23: windows
            # ssh.connect(hostname='192.168.101.23', port=22, username='TEST\Administrator', password='Win*Server')
            # 执行命令
            shell = ssh.invoke_shell()
